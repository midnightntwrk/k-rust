...
/// Runtime hooks that are allowed to enter the in-process evaluator dispatcher.
///
/// Keep this list explicit: capability audits compare it with the hooks declared by
/// semantics, and the dispatcher gate below makes an unregistered match arm unreachable.
pub const EVALUATOR_HOOKS: &[&str] = &[
    "BOOL.and",
    "BOOL.andThen",
    "BOOL.eq",
    "BOOL.implies",
...
fn evaluate_hook_with_sort(
    ...
    check_interrupted()?;
    if !EVALUATOR_HOOKS.contains(&hook) {
        return Ok(BuiltinResult::NotApplicable);
    }
    match hook {
