...
fn parsed_definition_for_json(
    ...
    let mut modules_with_visible_bubbles = source_modules
        ...
        .collect::<BTreeSet<_>>();
    loop {
        // Bubble visibility is transitive through the raw graph. Using the resolved graph here
        // would miss imports removed by backend tags, while Java computes this set first.
...
fn run_backend(
    ...
    if let Some(search) = options.search {
        let target = match search.pattern {
            ...
            None => Pattern {
                // Match the default search variable emitted by reference K's `krun`.
