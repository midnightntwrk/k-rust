...
mapfile -t available < <(
  jq -r '.execution[] | select(.name != "imp") | .name' <<<"$manifest_json"
)
selected=("${available[@]}")
if (($#)); then
  selected=("$@")
...
for name in "${selected[@]}"; do
  if ! printf '%s\n' "${available[@]}" | grep -Fxq "$name"; then
    echo "error: unknown non-IMP execution case: $name" >&2
...
echo "reference bounded non-IMP execution differential corpus passed"
